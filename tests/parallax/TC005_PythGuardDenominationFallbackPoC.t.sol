// SPDX-License-Identifier: GPL-3.0
pragma solidity 0.8.28;

import { PythAdaptor } from "contracts/oracles/adaptors/pyth/PythAdaptor.sol";
import { ICentralRegistry } from "contracts/interfaces/ICentralRegistry.sol";
import { IOracleAdaptor } from "contracts/interfaces/IOracleAdaptor.sol";

import { TestBaseOracleManager } from "tests/oracles/OracleManager/TestBaseOracleManager.sol";

contract NativeUsdOracle {
    address internal constant _ETH_ADDRESS =
        0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

    function isSupportedAsset(address asset) external pure returns (bool) {
        return asset == _ETH_ADDRESS;
    }

    function getPrice(
        address asset,
        bool inUSD,
        bool getLower
    ) external pure returns (IOracleAdaptor.PricingResult memory result) {
        if (asset != _ETH_ADDRESS) {
            return IOracleAdaptor.PricingResult(0, true, true);
        }

        if (!inUSD) {
            return IOracleAdaptor.PricingResult(1e18, false, false);
        }

        if (getLower) {
            return IOracleAdaptor.PricingResult(3900e18, true, false);
        }

        return IOracleAdaptor.PricingResult(4000e18, true, false);
    }
}

contract TC005PythGuardDenominationFallbackPoC is TestBaseOracleManager {
    address internal constant _PYTH_ADDRESS =
        0x4305FB66699C3B2702D4d05CF36551390A4c69C6;

    PythAdaptor public adaptor;
    NativeUsdOracle public nativeOracle;

    receive() external payable {}

    function setUp() public override {
        super.setUp();

        nativeOracle = new NativeUsdOracle();
        oracleManager.addApprovedAdaptor(address(nativeOracle));
        oracleManager.addAssetPricingAdaptor(
            _ETH_ADDRESS,
            address(nativeOracle),
            100,
            50,
            100,
            50
        );

        adaptor = new PythAdaptor(
            ICentralRegistry(address(centralRegistry)),
            address(0),
            _PYTH_ADDRESS,
            _WETH_ADDRESS
        );

        oracleManager.addApprovedAdaptor(address(adaptor));

        adaptor.addAsset(
            _WBTC_ADDRESS,
            true,
            1 days,
            0xe62df6c8b4a85fe1a67db44dc12de5db330f7ac66b72dc658afedf0f4a415b43
        );

        vm.warp(1711335100);
    }

    function test_tc005_pythNativeGuardCanBeStoredYetStayInertWhenOnlyUsdFeedIsConfigured() public {
        _updatePythWbtcPrice();

        oracleManager.addAssetPricingAdaptor(
            _WBTC_ADDRESS,
            address(adaptor),
            100,
            50,
            100,
            50
        );

        (uint256 nativePriceBefore, uint256 errorBefore) = oracleManager.getPrice(
            _WBTC_ADDRESS,
            false,
            false
        );
        assertEq(errorBefore, 0, "tc005:expected-clean-native-price");
        assertGt(nativePriceBefore, 0, "tc005:missing-native-price");

        uint256 guardCap = nativePriceBefore / 2;
        adaptor.setGuardedPriceConfig(
            _WBTC_ADDRESS,
            false,
            0,
            0,
            guardCap,
            0
        );

        IOracleAdaptor.PriceGuard memory storedGuard = adaptor.getPriceGuard(
            _WBTC_ADDRESS,
            false
        );
        assertEq(
            storedGuard.basePrice,
            guardCap,
            "tc005:expected-native-guard-to-be-stored"
        );

        (uint256 nativePriceAfter, uint256 errorAfter) = oracleManager.getPrice(
            _WBTC_ADDRESS,
            false,
            false
        );
        assertEq(errorAfter, 0, "tc005:expected-clean-native-price-after-guard");
        assertEq(
            nativePriceAfter,
            nativePriceBefore,
            "tc005:native-price-changed-even-though-runtime-fell-back-to-usd"
        );
        assertGt(
            nativePriceAfter,
            guardCap,
            "tc005:native-guard-cap-should-have-clamped-if-it-bound-runtime"
        );
    }

    function _updatePythWbtcPrice() internal {
        bytes[] memory priceUpdateData = new bytes[](1);
        priceUpdateData[
            0
        ] = hex"";

        adaptor.updateFeedsWithNative{ value: 1 ether }(priceUpdateData);
    }
}
