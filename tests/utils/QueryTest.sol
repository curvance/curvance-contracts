...
library QueryTest {
    ...
    function buildSolanaPdaRequestBytes(
        ...
        for (uint idx; idx < numPdas; ) {
            result = abi.encodePacked(result, _pdas[idx]);
    ...
    function buildSolanaPdaEntry(
        ...
        return abi.encodePacked(_programId, _numSeeds, _seeds);
    }
    ...
    function buildSolanaPdaSeedBytes(
        ...
        for (uint idx; idx < numSeeds; ) {
            ...
            result = abi.encodePacked(
                result,
                abi.encodePacked(uint32(seedLen), _seeds[idx])
            );
    ...
    function buildEthCallResultBytes(
        ...
        return abi.encodePacked(uint32(_result.length), _result);
    }
