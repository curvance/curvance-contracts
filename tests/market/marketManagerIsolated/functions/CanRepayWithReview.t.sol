...
contract CanRepayWithReviewTest is TestBaseMarketIsolated {
    ...
    function test_canRepayWithReview_fail_whenCallerIsNotCToken() public {
        vm.expectRevert(MarketManagerIsolated.MarketManager__Unauthorized.selector);
        marketManagerIsolated.canRepayWithReview(address(borrowableCUSDC), 1, address(usdc), 6, user1);
    }

    function test_canRepayWithReview_fail_whenUserMulticallsAsMarketManager() public {
        ...
        calls[0].data = abi.encodeCall(
            ...
            (address(borrowableCUSDC), 1, address(usdc), 6, user1)
        );
