...
import { console2 } from "forge-std/console2.sol";
  ...
  // Tests that protocol-derived incentive + custom close factor works correctly.
...
contract TestZeroIncentiveNonZeroCloseFactor is TestBaseMarketIsolated {
    ...
    function test_success_AuctionLiquidationWithZeroIncentiveAndNonZeroCloseFactor() public {
        _prepareUSDC(auctionPermsUser, 1000e6);

        vm.startPrank(auctionPermsUser);
        usdc.approve(address(borrowableCUSDC), 1000e6);
        // Set auction with zero incentive but custom non-zero close factor
        // closeFactorMin = 2000, closeFactorMax = 5000 (from _setCTokenConfigBasic), so use 3000 (30%)
        uint256 customCloseFactor = 3000;
        marketManagerIsolated.setTransientLiquidationConfig(address(borrowableCDAI), 0, customCloseFactor);
        ...
        {
            // Get protocol liquidation config to calculate expected incentive
            (uint256 liqIncBase, uint256 liqIncCurve, , , , ,,)
                = marketManagerIsolated.liquidationConfig(address(borrowableCDAI));

            // Calculate expected protocol-derived liquidation incentive
        ...
        // Execute liquidation with protocol-derived incentive + custom close factor
