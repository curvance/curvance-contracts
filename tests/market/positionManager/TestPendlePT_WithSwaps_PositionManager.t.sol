...
contract TestPendlePT_WithSwaps_PositionManager is TestBaseMarketIsolated {
    ...
    address positionManagerAddress =
        0x27cc01A4676C73fe8b6d0933Ac991BfF1D77C4da;
    ...
    function _createLeverage() public {
        ...
        action.approx.guessMin = 0.001e18;
        action.approx.guessMax = 10.0e18;
        action.approx.guessOffchain = 1.0e18;
    ...
    function testLeverageWithPreSwap_Success() public {
        ...
        action.approx.maxIteration = 30;
        action.approx.eps = 1e15;
        action.input.tokenIn = _USDC_ADDRESS;
        action.input.netTokenIn = estimatedUniswapOutputAmount;
        action.input.tokenMintSy = _STETH;
        action.input.pendleSwap = _PENDLE_SWAP;
        action.input.swapData.swapType = SwapType.KYBERSWAP;
        action
            .input
    ...
    function testDeLeverageWithPostSwap_Success() public {
        ...
        vm.startPrank(user);
        AccountSnapshot memory borrowableCDAIBeforeSnapshot = borrowableCDAI
            .getSnapshot(user);
        AccountSnapshot memory cPendlePTSTETHBeforeSnapshot = cPendlePTSTETH
            .getSnapshot(user);

        PendlePTPositionManager.DeleverageAction
            memory deleverageAction = _buildPostSwapDeleverageAction(
        ...
        assertEq(
            repaid,
            _EXPECTED_DELEVERAGE_REPAID_DAI,
    ...
    function testRevert_DeLeveragePostSwapInputTokenMustMatchPendleOutput()
        public
    {
        _setUpMarket(1);
        _createLeverage();
        ...
        deleverageAction.swapActions[0].inputToken = _DAI_ADDRESS;
        ...
        positionManager.deleverage(deleverageAction, 0.6e18);

        AccountSnapshot memory borrowableCDAIAfterSnapshot = borrowableCDAI
            .getSnapshot(user);
        AccountSnapshot memory cPendlePTSTETHAfterSnapshot = cPendlePTSTETH
            .getSnapshot(user);
        assertEq(
    ...
    function testRevert_DeLeveragePostSwapOutputMustReachDebtAsset() public {
        _setUpMarket(1);
        _createLeverage();
    ...
    function _buildPostSwapDeleverageAction(
        uint256 debtBalance
    )
        internal
        view
        ...
        )
    {
        ...
        action.output.tokenOut = _STETH;
        action.output.tokenRedeemSy = _STETH;
