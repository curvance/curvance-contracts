...
import {
    KuruCalldataChecker
} from "contracts/calldata-checker/swap-checker/KuruCalldataChecker.sol";
...
contract TestSimplePositionManagerMonadWithSwaps is TestBaseMarketIsolated {
    ...
    function setUp() public override {
        ...
        kyberSwapChecker = new KyberSwapChecker(
            kyberSwapRouter, kyberSwapExecutors, address(centralRegistry)
        );
        centralRegistry.setExternalCalldataChecker(
            kyberSwapRouter, address(kyberSwapChecker)
        );
        ...
        centralRegistry.setExternalCalldataChecker(
            kuruRouter, address(kuruSwapChecker)
        );
        ...
        ChainlinkAdaptor chainlinkAdaptor =
            new ChainlinkAdaptor(ICentralRegistry(address(centralRegistry)));
        oracleManager.addApprovedAdaptor(address(chainlinkAdaptor));

        chainlinkAdaptor.addAsset(
            _USDC_ADDRESS, true, address(chainlinkUSDC_USD), 0
        );
        ...
        oracleManager.addAssetPricingAdaptor(
            WMON_ADDRESS, address(chainlinkAdaptor), 100, 50, 100, 50
        );
        ...
        marketManagerIsolated.listTokens(
            address(borrowableCUSDC_MONAD), address(borrowableCWMON)
        );
        ...
        protocolReader =
            new ProtocolReader(ICentralRegistry(address(centralRegistry)));
    ...
    function test_liveForkPositionManagerSwapFlows() public {
        uint256 baselineSnapshot = vm.snapshotState();
        _assertLeverage_TestSimplePositionManagerMonadWithSwaps();
        assertTrue(
            vm.revertToState(baselineSnapshot),
            "simple-pm:failed-to-revert-leverage-baseline"
        );

        baselineSnapshot = vm.snapshotState();
        _assertDeleverage_TestSimplePositionManagerMonadWithSwaps();
        assertTrue(
            vm.revertToState(baselineSnapshot),
            "simple-pm:failed-to-revert-deleverage-baseline"
        );

        baselineSnapshot = vm.snapshotState();
        _assertDeleverage_refundsExcessDebtAssetWhenLiveDebtIsClipped();
        assertTrue(
            vm.revertToState(baselineSnapshot),
            "simple-pm:failed-to-revert-refund-baseline"
        );

        _assertDeleverage_fail_whenBelowMinLoan();
    }

    function _assertLeverage_TestSimplePositionManagerMonadWithSwaps()
        internal
    {
    ...
    function _assertDeleverage_TestSimplePositionManagerMonadWithSwaps()
        internal
    {
        _assertLeverage_TestSimplePositionManagerMonadWithSwaps();
        ...
        assertLt(
            debtAfter, debtBefore, "Debt should decrease after deleverage"
        );
    }

    function _assertDeleverage_refundsExcessDebtAssetWhenLiveDebtIsClipped()
        internal
    {
        _assertLeverage_TestSimplePositionManagerMonadWithSwaps();
        ...
        vm.startPrank(user1);
        IERC20(_USDC_ADDRESS)
            .approve(address(borrowableCUSDC_MONAD), liveDebt - residualDebt);
    ...
    function _assertDeleverage_fail_whenBelowMinLoan() internal {
        _assertLeverage_TestSimplePositionManagerMonadWithSwaps();
        ...
        vm.startPrank(user1);
        vm.expectRevert(
            LiquidityManagerIsolated.LiquidityManager__InsufficientLoanSize
                .selector
        );
