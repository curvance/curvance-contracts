...
contract TestPendlePtAggregator is TestBaseOracleManager {
    ...
    address internal PT_weETH_25JUN2026 =
        0x8E8b8d3b2DcA78cb04B9914b7EC1Ad72F671f96D;
    address internal CHAINLINK_ETH_USD =
        0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419;
    address internal CHAINLINK_weETH_ETH =
    ...
    function test_success_getPrice() public {
        ...
        oracleManager.addAssetPricingAdaptor(
            PT_weETH_25JUN2026, address(chainlinkAdaptor), 100, 50, 100, 50
        );
        ...
        (, int256 rawEthUsd,,,) =
            aggregator.underlyingAggregator().latestRoundData();
        ...
        assertTrue(
            adjustedAnswer < rawEthUsd,
            "PT should be priced below underlying ETH"
        );
        ...
        int256 expectedAnswer =
            (rawEthUsd * int256(exchangeRate)) / int256(WAD);
        assertEq(
            adjustedAnswer,
            expectedAnswer,
            "Adjusted answer should match discount math"
        );

        assertTrue(ptWeETH_USD_Price > 0, "PT USD price should be positive");
    }

    function test_success_postExpiryPricesOneToOneWithUnderlying() public {
        _deployAggregatorCorrectly();
        ...
        assertEq(
            adjustedAnswer,
            rawEthUsd,
            "expired PT aggregator MUST price 1:1 with underlying"
        );
        assertEq(
            aggregator.getAdjustedAnswer(rawEthUsd),
            rawEthUsd,
