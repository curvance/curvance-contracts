...
contract AerodromeVolatilePToken is PTokenCompounding {
    /// TYPES ///
    ...
    error AerodromeVolatilePToken__ChainIsNotSupported();
    error AerodromeVolatilePToken__StakingTokenIsNotAsset(
        address stakingToken
    );
    error AerodromeVolatilePToken__AssetIsNotStable();
    error AerodromeVolatilePToken__SlippageError();
    error AerodromeVolatilePToken__InvalidSwapData();
    ...
    constructor(
        ...
        if (block.chainid != 8453) {
            revert AerodromeVolatilePToken__ChainIsNotSupported();
        }
        ...
        if (gauge.stakingToken() != _asset) {
            revert AerodromeVolatilePToken__StakingTokenIsNotAsset(
                gauge.stakingToken()
            );
        }
        ...
        if (IVeloPool(_asset).stable()) {
            revert AerodromeVolatilePToken__AssetIsNotStable();
        }
    ...
    function harvest(
        ...
        if (_checkVestStatus(_vaultData)) {
                ...
                if (rewardAmount > 0) {
                    ...
                    if (!rewardTokenIsUnderlying) {
                        ...
                        if (!isApprovedAsset[swapData.inputToken]) {
                            ...
                            revert AerodromeVolatilePToken__InvalidSwapData();
                        }
            ...
            if (totalAmountA == 0) {
                revert AerodromeVolatilePToken__SlippageError();
            }
