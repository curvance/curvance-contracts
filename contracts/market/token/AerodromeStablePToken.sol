...
contract AerodromeStablePToken is CompoundingPToken {
    ...
    error AerodromeStablePToken__ChainIsNotSupported();
    error AerodromeStablePToken__StakingTokenIsNotAsset(address stakingToken);
    error AerodromeStablePToken__AssetIsNotStable();
    error AerodromeStablePToken__SlippageError();
    error AerodromeStablePToken__InvalidSwapData();
    ...
    constructor(
        ...
        if (gauge.stakingToken() != _asset) {
            revert AerodromeStablePToken__StakingTokenIsNotAsset(
                gauge.stakingToken()
            );
        }
        ...
        if (!IVeloPool(_asset).stable()) {
            revert AerodromeStablePToken__AssetIsNotStable();
        }
    ...
    function harvest(
        ...
        if (_checkVestStatus(_vaultData)) {
                ...
                if (rewardAmount > 0) {
                    ...
                    if (!rewardTokenIsUnderlying) {
                        ...
                        if (!isApprovedAsset[swapData.inputToken]) {
                            ...
                            revert AerodromeStablePToken__InvalidSwapData();
                        }
            ...
            if (totalAmountA == 0) {
                revert AerodromeStablePToken__SlippageError();
            }
