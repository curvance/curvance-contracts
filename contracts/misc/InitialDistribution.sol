...
contract InitialDistribution is ReentrancyGuard {
    ...
    error InitialDistribution__NotEligible();
    ...
    constructor(
        ...
        if (maximumClaimAmount_ * lockedClaimMultiplier > 15750002.59 ether) {
            revert InitialDistribution__InvalidlockedClaimMultiplier();
        }
    ...
    function rescueToken(address token, uint256 amount) external {
        ...
        if (token == cve) {
            revert InitialDistribution__TransferError();
        }
