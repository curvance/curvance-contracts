...
contract LBP {
    ...
    error LBP__InvalidPriceSource();
    error LBP__NotStarted();
    error LBP__AlreadyStarted();
    error LBP__InSale();
    error LBP__Closed();
    error LBP__Success();
    ...
    function start(
        ...
        if (startTime != 0) {
            revert LBP__AlreadyStarted();
        }

        if (startTimestamp < block.timestamp) {
            revert LBP__InvalidStartTime();
        }

        if (softPriceInUSD >= hardPriceInUSD) {
            revert LBP__InvalidPrice();
        }
        ...
        // the payment token.
        if (errorCode == 2) {
            revert LBP__InvalidPriceSource();
        }
